...
def main(model_filepath, result_filepath, scratch_dirpath, examples_dirpath, example_img_format='png'):
    ...
    elif model_type == 'AlexNet' or model_type == 'ConvNet':
        children = list(model.children())
        target_layers = ['Sequential']
    else:
...
def save_trigger_images(args):
    from torchvision.utils import save_image
    img = torch.load("a_dc_cifar_img.pth")
    for idx, i in enumerate(img):
        if idx >= 10:
            break
        save_image(i, os.path.join(args.examples_dirpath, "{}_0_0_0.png".format(idx)), format="png")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_filepath', type=str, help='File path to the pytorch model file to be evaluated.', default='./a_dc_cifar.pth')
